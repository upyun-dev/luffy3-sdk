  ...
  constructor(baseUrl, accessKey, accessSecret) {
    ...
    this.rp = got.extend({
      ...
      hooks: {
        beforeRequest: [
          async (request) => {
            const {timestamp, CanonicalizedQueryString} = sign.getCanonicalizedQueryString(this.accessKey, request.searchParams);
...
LuffyThree.prototype.createService = async function (project, options = {}) {
  return await this.rp({
    method: 'POST',
    url: `project/${project}/containerapps?accessKey=${this.accessKey}`,
    json: options,
  });
};
...
LuffyThree.prototype.destroyService = async function (project, app) {
  return await this.rp({
    method: 'DELETE',
    url: `project/${project}/containerapp/${app}?accessKey=${this.accessKey}`,
  });
};
...
LuffyThree.prototype.restartService = async function (project, app) {
  return await this.rp({
    method: 'POST',
    url: `project/${project}/containerapp/${app}/restart?accessKey=${this.accessKey}`,
  });
};
...
LuffyThree.prototype.listPods = async function (project, app, options = {}) {
  return await this.rp({
    method: 'GET',
    url: `project/${project}/containerapp/${app}/pods?accessKey=${this.accessKey}`,
    searchParams: options,
  });
};
